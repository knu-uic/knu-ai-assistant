...
`/api/mcp`는 호스트 loopback Caddy 게이트웨이(`127.0.0.1:8000`)로만 제공되는 내부
데모용 stateless Streamable HTTP MCP 경로다. 게이트웨이가 서버의
`MCP_AUTH_TOKEN`을 Bearer 헤더로 주입하므로, 호출자는 token을 보내지 않으며 tool
인자나 모델 대화에 넣지 않는다. 게이트웨이가 아닌 API MCP 경로는 올바른 Bearer
token 없이는 거부된다.
