...
def _refresh_learningx_session_with_token(token: str, state_path: Path, lms_url: str) -> bool:
    ...
    try:
        ...
        courses_url = build_url(lms_url, "/api/v1/courses")
        ...
        launch_res = requests.get(
            launch_api_url,
            headers=headers,
            params={"url": build_url(lms_url, "/learningx/login")},
            timeout=10
        )
    ...
    for course_id, course_name in courses.items():
        ...
        if learningx is not None:
            try:
                ...
                token_path = canvas_token_path(Path(DEFAULT_STATE_PATH))
                if token_path.exists():
