...
→ G1 조회 성공
→ RAW RESULT 성공
→ TimetableData.fromJson 성공
→ saveTimetableCache() 누락
→ UI에서 cache 읽기 실패
...
RAW RESULT 로그가 성공했다면

이미:

KNUIS reverse engineering 자체는 성공
...
새로운 기능 추가 시 Reverse Engineering 흐름

새 기능 추가 시 항상 반복되는 흐름:
