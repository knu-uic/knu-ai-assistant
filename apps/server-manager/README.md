...
임베딩 모델 교체는 기존 벡터를 먼저 삭제하지 않는다. Manager가 새 모델의
벡터를 모델별 별도 행으로 끝까지 생성하고, 전부 성공한 뒤에만 활성 검색 모델을
원자적으로 바꾼다. 실패하거나 앱이 종료되면 기존 모델이 계속 검색을 담당하며,
완성되지 않은 새 모델 행은 같은 교체 작업을 다시 실행해 덮어쓸 수 있다. 현재
인덱스와 차원이 다른 모델은 연결 테스트 단계에서 차단한다.
