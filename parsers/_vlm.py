"""VLM 공용 호출 유틸. curriculum_parser 등에서 import.

설계:
- `image_to_text(bytes, mime, prompt, model)` 단일 진입점. 호출자가 prompt + model 지정.
- `_client(model)` 은 모델별 1 인스턴스 lru_cache.
...
from config import GOOGLE_API_KEY, VLM_PROVIDER, OPENAI_API_KEY, LLM_MODEL
...
def image_to_text(image_bytes: bytes, mime: str, prompt: str, model: str = LLM_MODEL) -> str:
    ...
    msg = HumanMessage(content=[
        ...
        _image_block(VLM_PROVIDER, data_url),
    ])
