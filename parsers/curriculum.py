...
3. 행 규칙 (반드시 준수):
   ...
   - 이수구분: 표의 첫 번째 '구분' 열(전공필수/전공선택/교양 등)을 채운다.
     * 셀이 상하로 크게 병합되어 있는 경우, "전공선택" 같은 텍스트가 셀의 맨 아래나 구석에 치우쳐 적혀 있더라도, 해당 병합 범위 전체에 속하는 모든 과목들의 이수구분을 동일하게 "전공선택"으로 매핑하라.
   - 학점: 그 과목의 학점.
   - 학년/학기: 표가 매트릭스(행렬) 형태이므로, 각 과목의 학점(숫자 3, 1 등)이 위치한 칸의 세로 구분선을 헤더(1학년 1/2, 2학년 1/2 등)와 정렬하여 추적하라.
     * 첫 번째 학기 열인 '1학년 1학기(1)'는 '과목명' 바로 다음의 첫 번째 숫자 열이다.
     * '데이터 분석과 AI의 첫걸음'은 과목명 바로 옆 첫 번째 열이므로 '1-1'이다. 이와 같이 수직 정렬을 바르게 맞추어 정리하라.
4. 예외 금지: 해상도가 다소 낮아 보이더라도 최대한 비전 판독을 시도하여 단 하나의 과목도 빠뜨리지 말고 표 전체를 끝까지 완성하여 출력하라. 중간에 요청을 거절하거나 추가 정보를 요구하지 말라.
