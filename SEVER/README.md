...
├── APP_PAGE/                      # 홈, 공지, 챗봇, 프로필 페이지
├── config.py                      # 전역 runtime/config 관리
├── CRAWLERS/
│   ├── registry.py                # 크롤러 등록
...
→ RETRIEVAL/graph.py에서 chunk-level retrieval → reranker → evidence chunk 추출 → answerer 실행
...
2. `search_chunks()`가 카테고리별 chunk 테이블 전체에서 vector similarity top-N chunk를 검색합니다.
...
7. budget이 충분하면 support document의 원문 본문을 가능한 한 많이 유지합니다.
   - 1등 문서는 body full을 우선 보존하고, 2~3등 문서는 summary/evidence 우선 전략을 사용합니다.
...
→ vector top-K chunks
→ CrossEncoder rerank
→ evidence top-N chunks 추출
→ support document top-N 선정
→ context packing
```

실제 값은 `.env` / `config.py`에서 관리합니다.

답변/정제 단계의 context budget은 `config.py` 기반으로 관리합니다.
...
retrieval은 chunk 단위로 수행되지만, answerer에는 evidence chunk + support document context가 함께 전달됩니다.
...
python3 -m py_compile \
  streamlit_app.py main.py db.py model.py refine.py schema.py config.py \
  ...
  CRAWLERS/crawltest/crawl_one.py
```
