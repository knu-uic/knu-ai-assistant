...
| # | 서비스 | 실행 위치 | 명령 |
|---|--------|-----------|------|
| 1 | db (postgres+pgvector) | `services/api/` | `docker compose up -d db` |
| 2 | redis (잡 큐) | `services/api/` | `docker compose up -d redis` |
| 3 | api (FastAPI) | `services/api/` | `python -m uvicorn api.main:app --port 8000` |
| 4 | worker (arq — 동기화·공지 수집) | `services/api/` | `arq workers.arq_worker.WorkerSettings` |
| 5 | web (React 개발 서버) | `apps/web/` | `npm run dev` |
...
- **api가 부팅에서 죽음** → db·redis가 먼저 떠 있는지 확인 (`docker ps`).
  `.env`·`.secrets/`가 `services/api/` 바로 아래 있는지도 확인 (다른 위치면 env 누락으로 죽음).
